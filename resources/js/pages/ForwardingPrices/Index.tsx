...
export default function ForwardingPricesIndex() {
  const { forwardingPrices, filters } = usePage().props as any
  ...
  const handleReset = () => {
    ...
    setServiceType('')
    setPerPage(50)
    router.get('/forwarding-prices', { per_page: 50 }, { preserveScroll: true })
  }

  const handleDelete = (id: number) => {
    if (window.confirm('Are you sure you want to delete this forwarding price?')) {
      router.delete(`/forwarding-prices/${id}`)
    }
  }

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Forwarding Prices" />
      <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <Link href="/forwarding-prices/create" className="ml-auto">
                <Button variant="default">
                  <Plus className="mr-2 h-4 w-4" />
                  Add Forwarding Price
                </Button>
              </Link>
            </div>
          </CardContent>
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Showing <strong>{forwardingPrices.data.length}</strong> of <strong>{forwardingPrices.total}</strong> forwarding
            prices
          </p>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y">
                  {forwardingPrices.data.map((price: ForwardingPrice) => (
                    <tr key={price.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm">
                          {price.origin_country} → {price.destination_country}
                        </div>
                      </td>
                      <td className="whitespace-nowrap px-6 py-4">
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <span className="text-sm font-medium">{price.transit_days} days</span>
                      </td>
                      <td className="whitespace-nowrap px-6 py-4">
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <div className="flex gap-2">
                          <Link
                            href={`/forwarding-prices/${price.id}/edit`}
        ...
        {/* Pagination */}
        {forwardingPrices.last_page > 1 && (
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Page {forwardingPrices.current_page} of {forwardingPrices.last_page}
            </p>
            <div className="flex gap-2">
              {forwardingPrices.prev_page_url && (
                <Link href={forwardingPrices.prev_page_url} className="rounded border border-gray-300 px-3 py-2 hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-900/50">
                  Previous
                </Link>
              )}
              {forwardingPrices.next_page_url && (
                <Link href={forwardingPrices.next_page_url} className="rounded border border-gray-300 px-3 py-2 hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-900/50">
                  Next
                </Link>
              )}
            </div>
